...
class SSHClient:
    ...
    def create_session(self, host: str, port: int = 22, username: str = "root", password: str | None = None) -> dict:
        """手动创建 SSH 会话"""
    ...
    def get_or_create_session(
        self, host: str, port: int = 22, username: str = "root", password: str | None = None
    ) -> str:
        """
        获取或创建 SSH 会话（智能复用）
    ...
    def connect_host(self, host_id: str, terminal_name: str | None = None) -> dict:
    ...
    def run_command(self, session_id: str, command: str, timeout: int = 30, process: bool = False) -> dict:
        """
        在指定会话执行命令
    ...
    def exec_host(self, host_id: str, command: str, timeout: int = 30, process: bool = False) -> dict:
        """
        ...
        ip_match = []
        for t in terminals:
            if t.get("host") == ip_to_match and (port_to_match is None or t.get("port", 22) == port_to_match):
                ip_match.append(t)
        if ip_match:
        ...
        try:
            ...
            for h in hosts_data.get("hosts", []):
                if h.get("host") == ip_to_match and (port_to_match is None or h.get("port", 22) == port_to_match):
                    hid = h["id"]
    ...
    def get_host_id_by_name(self, name: str) -> str | None:
        """根据主机名称获取 host_id"""
    ...
    def sftp_upload(self, session_id: str, remote_path: str, local_path: str):
        ...
        with open(local_path, encoding="utf-8") as f:
            content = f.read()
        return self._request("POST", f"/api/sftp/{session_id}/write", {"path": remote_path, "content": content})
...
示例:
  ...
  python ssh_client.py upload abc12345 ./file.txt /tmp/file.txt  上传文件
    ...
    parser.add_argument("--server", default="http://127.0.0.1:8765", help="Server 地址 (默认: http://127.0.0.1:8765)")
    ...
    try:
        if args.subcommand == "hosts":
            ...
            for h in hosts:
                ...
                print(
                    f"{h['id']:<12} {h.get('name', ''):<16} {addr:<22} {h.get('type', ''):<8} {h.get('group', ''):<10} {h.get('terminal_count', 0):<4} {status:<6}"
                )
            ...
            for s in sessions:
                ...
                print(
                    f"{s['session_id']:<12} {addr:<22} {s['username']:<10} {'是' if s['connected'] else '否':<6} {'是' if s.get('has_shell') else '否':<6} {last:<20}"
                )
            ...
            for t in terminals:
                ...
                print(
                    f"{t['session_id']:<12} {t['host_name']:<18} {addr:<22} {t['terminal_name']:<10} {t.get('host_type', ''):<8}"
                )
