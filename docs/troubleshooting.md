...
- **SSH 连接层**断开的自动重连：后台每 15 秒检测连接状态，连接断开时自动重连并恢复 shell，最多 5 次
- **不自动重连的场景**（用户确认方向）：
  - 终端输入失败**不触发自动重连**（避免重连 close process 杀死 vi/vim 等正在运行的程序）
  - monitor 检测到 shell 状态异常**不自动重启**，只提示「点击顶部重连」
- 断开后的终端由**前端手动重连**：顶栏「🔗 重连」按钮
- 完整踩坑记录见 [development-lessons.md](development-lessons.md)
