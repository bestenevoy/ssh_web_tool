...
服务监听端口等启动参数通过 `config.json` 配置（首次启动自动生成，统一放在用户目录 `~/.ai4one/sshtool/`，`config.json` / `data.json` / `logs/` 均在该目录）：
...
- `config.json` 为本地配置（不入库），模板见 `config.example.json`；**只读写 `~/.ai4one/sshtool/` 一处**。程序首次运行会自动把旧位置（旧目录 `~/.ai4one/wstool/`、EXE 目录、项目根）已有的配置与数据迁移过来，仅复制不覆盖。
