...
const resyncTerminal = useCallback((session_id: string, term: Terminal, ws: WebSocket | null, clean_screen: boolean = true) => {
  ...
  const handleTerminalInput = useCallback((session_id: string, data: string) => {
    ...
    // 回车记录（全部记录通道均由"输入动作"触发；后端回显解析已于 2026-09-24
    // 降级为只做 cd 跟踪，不再写历史——从输出流猜命令在持续回显场景是最大
    // 误报源：tail -f/设备日志流的输出行撞提示符形态逐条入库，>3s 间隔的同
    // 一行还会反复 count+1，去重窗口救不回来）。
    // 键盘缓冲只在"纯键入无补全"时可信：
    // - 可信行（有键入内容且本行无 Tab）→ 键入版即时记录（最快最稳）
    // - 含 Tab 补全行 / 缓冲空但有输入锚行（↑ 历史召回、整段粘贴、自研 shell
    //   下行回显）→ captureTypedLine 从 xterm 缓冲捕获屏幕实际显示的命令行
    //   （所见即所得，不依赖提示符格式/重绘方式，S2 主通道）
    // - 捕获不可用（行被后台输出推挤等）且缓冲非空 → 延迟 2s 落键盘版
    //   （defer 判定现仅对本机 PSReadLine 采集器的权威记录有意义；SSH 下必落
    //   键盘版——可能残缺，但至少一条且不阻塞）
    if (isEnter(data)) {
